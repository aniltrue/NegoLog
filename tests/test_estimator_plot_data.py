...
def test_median_plots_receive_truncated_means_without_mutating_inputs(tmp_path, monkeypatch):
    ...
    assert (rmse, spearman, kendall) == original  # nosemgrep: python_assert_rule-assert-used
    assert plots[logger.get_path('opponent model/estimator_rmse')]['First'] == [2., 3., 7., 11.]  # nosemgrep: python_assert_rule-assert-used
    assert plots[logger.get_path('opponent model/estimator_rmse_until_median_round')]['First'] == [2., 3.]  # nosemgrep: python_assert_rule-assert-used
    assert plots[logger.get_path('opponent model/estimator_spearman_until_median_round')]['First'] == [.1, .2]  # nosemgrep: python_assert_rule-assert-used
    assert plots[logger.get_path('opponent model/estimator_kendall_tau_until_median_round')]['First'] == [.5, .6]  # nosemgrep: python_assert_rule-assert-used
